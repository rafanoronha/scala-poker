...
package object game {
  ...
  object Game extends StraightAwareness with FlushAwareness with PairsAwareness {
  ...
  trait PairsAwareness {
    def onePair(implicit cards:Cards) = 4 == cards.map(_.rank).groupBy(identity).size
  }
